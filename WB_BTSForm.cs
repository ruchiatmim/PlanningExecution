...
namespace Version3
{
    public partial class WB_BTSForm : Form
    {
        public WB_BTSForm()
        {
            InitializeComponent();
               ...
               private void ubSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ugBTSSerial.Rows.Count; i++)
            {
               ...
               // string asset_tag_num = ugBTSSerial.Rows[i].Cells["asset_tag_num"].Value.ToString();
                ...
                if (auto_id == 0)
                {
                    qry = qry + " insert into BTS_GoogleIntInvTranSerial(ProcessDate,app "+ /*,asset_tag_num*/",epi_tran_type,fileDate,filename,folder,location,parent_serial_num,part_no,reference,region,serial_num,status,tran_date,tran_id,uniqueIdent) select ProcessDate,'"+app+/*"','" + asset_tag_num +*/ "',epi_tran_type,fileDate, filename,folder,location,'" + parent_serial_num + "',part_no,reference,region,'" + serial_num + "','N',tran_date,tran_id,uniqueIdent  from BTS_GoogleIntInvTranWB where uniqueIdent='" + uniqueIdent + "'";
                }
                else
                {
