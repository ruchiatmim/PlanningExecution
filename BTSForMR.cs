...
namespace Version3
{
    public partial class BTSForMR : Form
    {
        public BTSForMR()
        {
            InitializeComponent();
       ...
       //     e.Layout.Bands[0].Columns["TranType"].ValueList = this.ddTranType;
        ...
        private void loadgrid()
        {
          ...
          //  sql = "select  [auto_id],[status],Replace(replace(Replace(Replace([filename],'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\INVTRAN\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\INVTRAN\\','') as filename,[fileDate],[ProcessDate],[dekit_summary_id] ,[tran_id],[uniqueIdent] ,[from_loc] ,[to_loc] ,[from_proj],[to_proj],[tran_date],[part_no],[tran_type],[tran_source],[tran_source_num] ,[tran_source_line],[serial_num] ,[qty] ,[reference]  ,[location]  from BTS_GoogleIntInvTran where  uniqueIdent is not null  ";  //,replace(filename,'\\\\mimsftp\\e$\\sftp\\msgGoogle\\PLATFORM\\PROD\\','') as file1
            ...
            sql = "SELECT [AutoId] ,[TranId]  ,uniqueIdent      ,[FromLoc]      ,[ToLoc]      ,[FromProj]      ,[ToProj]      ,[TranDate]      ,[PartNo]          ,[TranSourceNum]      ,[TranSourceLine]         ,[Qty]      ,[Reference]        ,[EPITranType]      ,[Plant]         ,[UTCdate]      ,[PoNum]      ,[PoLine],EDIStatus,ICNSentDate ,[EDIICN#] as ICN FROM  [dbo].[BTS_GoogleIntInvTranForMR]    where  uniqueIdent is not null     ";
        ...
        private void ubSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ugBTSSerial.Rows.Count; i++)
            {
                ...
                if (auto_id == 0)
                {
                    qry = qry + " insert into BTS_GoogleIntInvTranSerial([Status] ,[TranId],[uniqueIdent] ,[TranDate],[PartNo] ,[SerialNum] ,[ParentSerialNum] ,[Reference]      ,[AssetTagNum]      ,[FilePrefix]      ,[FileFolder]      ,[EPITranType]      ,[Plant]      ,[App]) select  'N', [TranId],[uniqueIdent] ,[TranDate] ,[PartNo] ,'"+serial_num+"','"+   parent_serial_num+"',[Reference],'', [FilePrefix] ,[FileFolder] ,[EPITranType]  ,[Plant]  ,[App]   from [dbo].[BTS_GoogleIntInvTranForMR] where uniqueIdent='" + uniqueIdent + "'";
                }
                else
                {
                    qry = qry + "   update dbo.BTS_GoogleIntInvTranSerial  set " + /* asset_tag_num='" + asset_tag_num + */  " parentserialnum='" + parent_serial_num + "', serialnum='" + serial_num + "' where autoid=" + auto_id.ToString();
                }
            }
