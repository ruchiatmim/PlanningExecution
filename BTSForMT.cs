...
namespace Version3
{
    public partial class BTSForMT : Form
    {
        public BTSForMT()
        {
            InitializeComponent();
        ...
        public void getSerialNum(string tranid)
        {
            
            //WebDataGridSerial.DataSource = null;
            string sql = "  SELECT [AutoId],Status,[SerialNum],[ParentSerialNum] ,[AssetTagNum],Reference  FROM [dbo].[BTS_GoogleIntInvTranSerial] where tranID='" + tranid + "'";
        ...
        private void ugBTS_AfterSelectChange(object sender, Infragistics.Win.UltraWinGrid.AfterSelectChangeEventArgs e)
        {
            if (this.ugBTS.Selected.Rows.Count > 0)
            {
                ...
                tranId = ugBTS.Selected.Rows[0].Cells["tranId"].Value.ToString().Trim();
            }
            else
            {
                tranId = "";
            }
            txtUniqueIdentifier.Text = tranId;
        ...
        private void loadgrid()
        {
          ...
          //  sql = "select  [auto_id],[status],Replace(replace(Replace(Replace([filename],'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\INVTRAN\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\INVTRAN\\','') as filename,[fileDate],[ProcessDate],[dekit_summary_id] ,[tran_id],[uniqueIdent] ,[from_loc] ,[to_loc] ,[from_proj],[to_proj],[tran_date],[part_no],[tran_type],[tran_source],[tran_source_num] ,[tran_source_line],[serial_num] ,[qty] ,[reference]  ,[location]  from BTS_GoogleIntInvTran where  uniqueIdent is not null  ";  //,replace(filename,'\\\\mimsftp\\e$\\sftp\\msgGoogle\\PLATFORM\\PROD\\','') as file1
            ...
            sql = "SELECT [AutoId]         ,[TranId]           ,[FromLoc]      ,[ToLoc]      ,[FromProj]      ,[ToProj]      ,[TranDate]      ,[PartNo]         ,[TranSourceNum]      ,[TranSourceLine]          ,[Qty]      ,[Reference]        ,[EPITranType]      ,[Plant]      ,EDIStatus   ,[UTCdate]      ,[PoNum]      ,[PoLine],isnull(EDIReference,'') as [EDI Reference],[EDIICN#] as ICN  ,ICNSentDate  FROM  [dbo].[BTS_GoogleIntInvTranForMT]    where  uniqueIdent is not null     ";
        ...
        private void ugBTS_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Key == "Save")
            {
              ...
              //  var_status = e.Cell.Row.Cells["status"].Value.ToString();
                ...
                var_tran_type = "MATERIAL TRANSFER";
                var_tran_source = "";
            ...
            //    var_poline =  Convert.ToInt32(e.Cell.Row.Cells["poline"].Value.ToString());
                ...
                string sqlstr = "exec [sp_updateBTSForMT] '" + var_auto_id.ToString() + "','N','" + var_tran_id + "','" + var_from_loc + "','" + var_to_loc + "','" + var_from_proj + "','" + var_to_proj + "','" + var_tran_date + "','" + var_part_no + "','" + var_tran_type + "','" + var_tran_source + "','" + var_tran_source_num + "','" + var_tran_source_line + "','" + var_serial_num + "','" + var_qty + "','" + var_reference + "','" + var_location + "','0','0','" + var_EDIreference + "','" + var_EDIStatus + "','" + var_utc_date + "'";
               UpdateRow(sqlstr);
            }
        }
        ...
        private void ubSave_Click(object sender, EventArgs e)
        {
            ...
            string tranid = txtUniqueIdentifier.Text;
            ...
            for (int i = 0; i < ugBTSSerial.Rows.Count; i++)
            {
                ...
                if (auto_id == 0)
                {
                    qry = qry + " insert into BTS_GoogleIntInvTranSerial([Status] ,[TranId] ,[TranDate],[PartNo] ,[SerialNum] ,[ParentSerialNum] ,[Reference]      ,[AssetTagNum]            ,[EPITranType]      ,[Plant]      ,[App]) select  'N', [TranId] ,[TranDate] ,[PartNo] ,'" + serial_num + "','" + parent_serial_num + "',[TranSourceNum],'' ,[EPITranType]  ,[Plant]  ,[App]   from [dbo].[BTS_GoogleIntInvTranForMT] where tranid='" + tranid + "'";
                }
                else
                {
