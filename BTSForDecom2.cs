...
namespace Version3
{
    public partial class BTSForDecom2 : Form
    {
        public BTSForDecom2()
        {
            InitializeComponent();
        ...
        private void BTSForDecom2_Load(object sender, EventArgs e)
        {
        ...
        private void LoadDropDown()
        {
            ...
            cmbPlant.DisplayMember = "loc";
            cmbPlant.ValueMember = "loc";
       ...
       //     e.Layout.Bands[0].Columns["TranType"].ValueList = this.ddTranType;
        ...
        private void loadgrid()
        {
          ...
          //  sql = "select  [auto_id],[status],Replace(replace(Replace(Replace([filename],'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\INVTRAN\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\INVTRAN\\','') as filename,[fileDate],[ProcessDate],[dekit_summary_id] ,[tran_id],[uniqueIdent] ,[from_loc] ,[to_loc] ,[from_proj],[to_proj],[tran_date],[part_no],[tran_type],[tran_source],[tran_source_num] ,[tran_source_line],[serial_num] ,[qty] ,[reference]  ,[location]  from BTS_GoogleIntInvTran where  uniqueIdent is not null  ";  //,replace(filename,'\\\\mimsftp\\e$\\sftp\\msgGoogle\\PLATFORM\\PROD\\','') as file1
            ...
            sql = "SELECT [AutoId]    ,[TranId]         ,[FromLoc]      ,[ToLoc]      ,[FromProj]      ,[ToProj]      ,[TranDate]      ,[PartNo]      ,[TranType]      ,[TranSource]      ,[TranSourceNum]      ,[TranSourceLine]      ,[SerialNum]      ,[Qty]      ,[Reference]        ,[EPITranType]      ,[Plant]         ,[UTCdate]      ,[PoNum]      ,[PoLine],EDIStatus  ,ICNSentDate,[EDIICN#]  as ICN  FROM  [dbo].[BTS_GoogleIntInvTranForDecom2]    where  uniqueIdent is not null     ";
            if (cmbStatus.Text != "ALL")            {
                sql = sql + " and   EDIstatus='" + cmbStatus.Value + "'";
            }

            if (txtSerialNum.Text.Trim() != "")
                sql = sql + " and   SerialNum like '" + txtSerialNum.Text.Trim() + "%'";
        ...
        private void ugBTS_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Key == "Save")
            {
               ...
               // if (e.Cell.Row.Cells["ponum"].Value.ToString().Trim() != "" && e.Cell.Row.Cells["ponum"].Value != null)
                var_ponum = e.Cell.Row.Cells["ponum"].Value.ToString();  
                 if  (e.Cell.Row.Cells["poline"].Value.ToString().Trim() !="" && e.Cell.Row.Cells["poline"].Value!=null)    
                var_poline =  Convert.ToInt32(e.Cell.Row.Cells["poline"].Value.ToString());

                 string sqlstr = "exec sp_updateBTSForDecom2 '" + var_auto_id.ToString() + "','" + var_status + "','" + var_tran_id + "','" + var_from_loc + "','" + var_to_loc + "','" + var_from_proj + "','" + var_to_proj + "','" + var_tran_date + "','" + var_part_no + "','" + var_tran_type + "','" + var_tran_source + "','" + var_tran_source_num + "','" + var_tran_source_line + "','" + var_serial_num + "','" + var_qty + "','" + var_reference + "','" + var_location + "','" + var_ponum + "','" + var_poline + "','" + var_EDIStatus + "','" + var_utc_date + "'";
               UpdateRow(sqlstr);
            }
        }
        ...
        private void btnSaveAll_Click(object sender, EventArgs e)
        {
              ...
              int var_auto_id;
                ...
                var_auto_id = Convert.ToInt32(row.Cells["autoid"].Value);
                     var_status = "N";// row.Cells["status"].Value.ToString();
                ...
                var_serial_num = row.Cells["serialnum"].Value.ToString();
                ...
                var_location = row.Cells["Plant"].Value.ToString();
                 if  (row.Cells["ponum"].Value.ToString() !="" && row.Cells["ponum"].Value!=null)
                var_ponum = Convert.ToInt32(row.Cells["ponum"].Value.ToString());  
                 if  (row.Cells["poline"].Value.ToString() !="" && row.Cells["poline"].Value!=null)    
                var_poline =  Convert.ToInt32(row.Cells["poline"].Value.ToString());
                 var_EDIStatus = row.Cells["EDIStatus"].Value.ToString();
                sqlstr =sqlstr +  "  exec sp_updateBTSForDecom2 '" + var_auto_id.ToString() + "','" + var_status + "','" + var_tran_id + "','" + var_from_loc + "','" + var_to_loc + "','" + var_from_proj + "','" + var_to_proj + "','" + var_tran_date + "','" + var_part_no + "','" + var_tran_type + "','" + var_tran_source + "','" + var_tran_source_num + "','" + var_tran_source_line + "','" + var_serial_num + "','" + var_qty + "','" + var_reference + "','" + var_location + "','" + var_ponum + "','" + var_poline + "','" + var_EDIStatus + "'";
