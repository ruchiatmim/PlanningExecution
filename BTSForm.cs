...
namespace Version3
{
    public partial class BTSForm : Form
    {
        public BTSForm()
        {
            InitializeComponent();
        ...
        {
            // TODO: This line of code loads data into the 'bTS.BTS_GoogleIntInvTran' table. You can move, or remove it, as needed.
           // this.bTS_GoogleIntInvTranTableAdapter.Fill(this.bTS.BTS_GoogleIntInvTran);
        ...
        {
            string sql = "select  *  from BTS_GoogleIntInvTran where  uniqueIdent is not null  ";  //,replace(filename,'\\\\mimsftp\\e$\\sftp\\msgGoogle\\PLATFORM\\PROD\\','') as file1

            if (cmbStatus.Text.ToString() !="ALL" )
            {
            ...
            if (this.txtStartdate.Value.ToString() != "" && this.txtEndDate.Value.ToString() != "")
            {
        ...
        private void ubSave_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < ugBTSSerial.Rows.Count; i++)
            {
               ...
               // string asset_tag_num = ugBTSSerial.Rows[i].Cells["asset_tag_num"].Value.ToString();
                ...
                if (auto_id == 0)
                {
                    qry = qry + " insert into BTS_GoogleIntInvTranSerial(ProcessDate,app "+ /*,asset_tag_num*/",epi_tran_type,fileDate,filename,folder,location,parent_serial_num,part_no,reference,region,serial_num,status,tran_date,tran_id,uniqueIdent) select ProcessDate,'"+app+/*"','" + asset_tag_num +*/ "',epi_tran_type,fileDate, filename,folder,location,'" + parent_serial_num + "',part_no,reference,region,'" + serial_num + "','N',tran_date,tran_id,uniqueIdent  from BTS_GoogleIntInvTran where uniqueIdent='" + uniqueIdent + "'";
                }
                else
                {
