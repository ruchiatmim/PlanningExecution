...
namespace Version3
{
    public partial class BTSMainForm : Form
    {
        public BTSMainForm()
        {
            InitializeComponent();
        ...
        private void ugBTS_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
          ...
          //  e.Layout.Bands[0].Columns["Status"].Style = Infragistics.Win.UltraWinGrid.ColumnStyle.DropDown;
            ...
            e.Layout.Bands[0].Columns["tran_type"].ValueList = this.ddTranType;
         //   e.Layout.Bands[0].Columns["Region"].ValueList = this.ddRegion;
        //    e.Layout.Bands[0].Columns["Folder"].ValueList = this.ddFolder;

            e.Layout.Bands[0].Columns["tran_source"].ValueList = this.ddSource;
        ...
        private void loadgrid()
        {
            ...
            sql = "select  [auto_id],[status],Replace(replace(Replace(Replace([filename],'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMUS\\outbound\\INVTRAN\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\TEMP\\',''),'\\\\mimsftp\\E$\\sftp\\msgGoogle\\PLATFORM\\PROD\\MIMIRE\\outbound\\INVTRAN\\','') as filename,[fileDate],[ProcessDate],[dekit_summary_id] ,[tran_id],[uniqueIdent] ,[from_loc] ,[to_loc] ,[from_proj],[to_proj],[tran_date],[part_no],[tran_type],[tran_source],[tran_source_num] ,[tran_source_line],[serial_num] ,[qty] ,[reference]  ,[location],isnull(EDI_reference,'') as [EDI Reference]  from BTS_GoogleIntInvTran where  uniqueIdent is not null  ";  //,replace(filename,'\\\\mimsftp\\e$\\sftp\\msgGoogle\\PLATFORM\\PROD\\','') as file1
        ...
        private void ugBTS_ClickCell(object sender, ClickCellEventArgs e)
        {
            ...
            if (e.Cell.Column.Key == "Save")
            {
                ...
                var_poline =  Convert.ToInt32(e.Cell.Row.Cells["poline"].Value.ToString());
                 var_EDIreference = e.Cell.Row.Cells["EDI Reference"].Value.ToString();

                 string sqlstr = "exec sp_updateBTS '" + var_auto_id.ToString() + "','" + var_status + "','" + var_tran_id + "','" + var_from_loc + "','" + var_to_loc + "','" + var_from_proj + "','" + var_to_proj + "','" + var_tran_date + "','" + var_part_no + "','" + var_tran_type + "','" + var_tran_source + "','" + var_tran_source_num + "','" + var_tran_source_line + "','" + var_serial_num + "','" + var_qty + "','" + var_reference + "','" + var_location + "','" + var_ponum + "','" + var_poline + "','" + var_EDIreference + "'";
                UpdateRow(sqlstr);
            }
        }
        ...
        private void btnSaveAll_Click(object sender, EventArgs e)
        {
            ...
            {
                if (row.Cells["auto_id"].Value.ToString() != "" && row.Cells["auto_id"].Value != null)
                    var_auto_id = Convert.ToInt32(row.Cells["auto_id"].Value);

                var_status = row.Cells["status"].Value.ToString();
                var_tran_id = Convert.ToInt32(row.Cells["tran_id"].Value.ToString());
                var_from_loc = row.Cells["from_loc"].Value.ToString();
                var_to_loc = row.Cells["to_loc"].Value.ToString();
                var_from_proj = row.Cells["from_proj"].Value.ToString();
                var_to_proj = row.Cells["to_proj"].Value.ToString();
                var_tran_date = Convert.ToDateTime(row.Cells["tran_date"].Value.ToString());
                var_part_no = row.Cells["part_no"].Value.ToString();
                var_tran_type = row.Cells["tran_type"].Value.ToString();
                var_tran_source = row.Cells["tran_source"].Value.ToString();
                var_tran_source_num = row.Cells["tran_source_num"].Value.ToString();
                var_tran_source_line = row.Cells["tran_source_line"].Value.ToString();
                var_serial_num = row.Cells["serial_num"].Value.ToString();
                var_qty = Convert.ToInt32(row.Cells["qty"].Value.ToString());
                var_reference = row.Cells["reference"].Value.ToString();
                var_location = row.Cells["location"].Value.ToString();

                if (row.Cells["ponum"].Value != "" && row.Cells["ponum"].Value != null)
                    var_ponum = Convert.ToInt32(row.Cells["ponum"].Value.ToString());
                if (row.Cells["poline"].Value != "" && row.Cells["poline"].Value != null)
                    var_poline = Convert.ToInt32(row.Cells["poline"].Value.ToString());
                
                var_EDIreference = row.Cells["EDI Reference"].Value.ToString();

                sqlstr = sqlstr + "   exec sp_updateBTS '" + var_auto_id.ToString() + "','" + var_status + "','" + var_tran_id + "','" + var_from_loc + "','" + var_to_loc + "','" + var_from_proj + "','" + var_to_proj + "','" + var_tran_date + "','" + var_part_no + "','" + var_tran_type + "','" + var_tran_source + "','" + var_tran_source_num + "','" + var_tran_source_line + "','" + var_serial_num + "','" + var_qty + "','" + var_reference + "','" + var_location + "','" + var_ponum + "','" + var_poline + "','" + var_EDIreference + "'";
